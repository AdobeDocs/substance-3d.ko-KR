...
이 기능은 깨끗한 텍스처를 달성하기 위해 베이킹 프로세스 동안 서로 지오메트리 출혈을 방지하는 데 매우 유용합니다. 동일한 결과를 얻기 위해 메쉬를 멀리 이동할 필요가 없습니다(종종 &quot;폭발&quot;이라고 함).
...
**뒷면 무시** 설정에 대해 이름별 일치 옵션을 활성화하면 단추 아래의 영역을 무시하면서 오클루전을 하나의 단수 상자로 보이게 할 수 있습니다.*이 결과는 [뒷면 무시] 설정을 사용했기 때문입니다.*
...
* 일부 베이커(예: [앰비언트 오클루전](../../bakers-settings/ambient-occlusion-from/ambient-occlusion-from-mesh.md))에서는 보조 광선을 생성하므로 이름별 보조 일치 설정이 사용될 수 있습니다.
