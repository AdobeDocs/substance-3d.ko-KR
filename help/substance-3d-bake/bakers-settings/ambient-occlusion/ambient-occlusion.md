...
앰비언트 오클루전 베이커를 사용하면 앰비언트 그림자 텍스처를 베이킹할 수 있습니다. 이 제빵사는 GPU에서 실행되는 빠른 알고리즘을 사용합니다.
...
> * 로우엔드/모바일 GPU에서 고해상도로 굽으면 충돌이 발생할 수 있습니다.

## 매개변수

| *이름* | *설명* |
| --- | --- |
| **표준 맵** | 베이커 계산 중에 고려될 메시 표면의 추가 형상 세부 정보를 제공하는 데 사용할 수 있는 표준 맵 파일을 입력합니다. 이 매개 변수는 선택 사항입니다. |
| **월드 스페이스** | 이 옵션을 활성화한 경우 입력 표준 맵이 탄젠트 공간(Tangent Space)이 아닌 월드 공간(World Space)에 있도록 지정합니다. 입력 표준 맵을 제공하지 않으면 이 매개 변수는 무시되거나 비활성화됩니다. |
| **표준 반전** | 반전된 표준으로 주변 오클루전 맵을 계산(Thickness 맵을 생성하는 데 사용할 수 있음). |
| **선택하지 않은 메시 부분 사용** | 메시의 선택되지 않은 메시 부분을 사용하여 주변의 오클루전 맵을 베이킹합니다. |
