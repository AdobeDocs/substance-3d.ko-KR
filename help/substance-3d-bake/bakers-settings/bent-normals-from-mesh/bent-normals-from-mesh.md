...
메시 베이커의 굽은 법선 은 주변 조명의 평균 방향을 설명하는 텍스처를 계산합니다. 이 베이커는 [메시](../../bakers-settings/ambient-occlusion-from/ambient-occlusion-from-mesh.md) 베이커의 앰비언트 오클루전에서 파생되었습니다.
...
| *매개 변수* | *설명* |
| --- | --- |
| **보조 광선** | 오클루전 광선의 양입니다. 값이 높으면 노이즈가 적게 발생하지만 계산에는 더 오래 걸립니다. |
| **최소 폐색기 거리** | 오클루전 광선이 높은 폴리 기하학에 닿을 최소 거리&#x200B;**.** |
| **최대 폐색기 거리** | 오클루전 광선이 높은 폴리 기하학에 도달하는 최대 거리입니다. |
...
| **맵 유형** | 출력 텍스처의 유형을 정의합니다.가능한 값:<ul data-preserve-html="true"><li data-preserve-html="true"><strong>월드 스페이스</strong></li><li data-preserve-html="true"><strong>탄젠트 공간</strong>(기본값)</li></ul> |
| **일반 방향** | **매트 유형**&#x200B;이(가) 탄젠트 공간(Tangent Space)으로 설정된 경우 출력 텍스처의 표준 형식을 제어합니다.가능한 값은 다음과 같습니다.<ul data-preserve-html="true"><li data-preserve-html="true"><strong>OpenGL</strong> <strong> <br/></strong></li><li data-preserve-html="true"><strong>DirectX</strong>(기본값)<strong> <br/></strong></li></ul> |
