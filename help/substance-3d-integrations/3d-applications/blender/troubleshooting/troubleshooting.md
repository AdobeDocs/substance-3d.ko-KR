...
시스템 콘솔을 사용하여 추가 기능을 사용하는 동안 발생한 오류를 진단할 수 있습니다. 블렌더의 시스템 콘솔 창은 운영 체제에 따라 다르게 열립니다. 자세한 지침은 Blender의 시스템 콘솔 [설명서 페이지](https://docs.blender.org/manual/en/2.79/advanced/command_line/introduction.html#console-window-status-and-error-messages)의 단계를 따르십시오. 콘솔 출력은 텍스처가 로드되지 않거나 재료가 처리 중 고착되는 등 예기치 않은 문제가 발생할 때 유용합니다.
...
* *순환 렌더링 보기에서 재질이 업데이트되지 않음*.
  * 기본적으로 추가 기능은 주기 렌더링 보기의 텍스처를 업데이트하지 않습니다. 그러나 추가 기능 환경 설정에서 <b>텍스처 자동 업데이트 주기</b>를 활성화하여 텍스처를 강제로 업데이트할 수 있습니다.
* 사이클 렌더링 보기에서 저장하는 동안 매개 변수가 되돌아갑니다.
  * 이것은 시각적 전용인 블렌더 측에서의 알려진 캐싱 문제입니다. 저장할 때 생성된 텍스처 파일을 업데이트하기 위해 원격 엔진으로 메시지를 보내지 않습니다. [주기 렌더링] 보기를 종료한 후 다시 전환하면 텍스처가 정상적으로 표시됩니다.
* *매개 변수를 실행 취소/변경한 후에는 재질이 더 이상 업데이트되지 않습니다.*
  * 작업을 취소한 후 재질이 업데이트되지 않을 수 있습니다. 매개 변수는 이전 상태로 되돌아가지만 텍스처는 일치하도록 실행 취소되지 않습니다. 텍스처를 다시 업데이트하려면 새로 고침 버튼을 사용하여 매개 변수를 기본값으로 되돌리고 텍스처를 다시 로드합니다.
* *Substance Designer에서 설정한 색상이 Blender의 색상 피커에서 약간 다르게 나타나며 색상 값이 동일하지 않습니다.*
  * Blender는 Blender 색상 피커의 색상에만 감마 교정을 적용합니다. 이로 인해 색상 피커가 일치하지 않는 경우에도 텍스처에 표시되는 색상은 Substance 앱에서 설정된 값에 따라 정확합니다.
