...
[생성된 텍스처]는 Substance을 생성하기 위해 Substance 엔진에서 계산한 텍스처의 출력을 표시합니다. 이러한 텍스처는 셰이더 입력으로 공급됩니다. 기본적으로 셰이더에 사용되는 기본 입력만 만들어집니다. &quot;모든 출력 생성&quot;이 활성화되어 있으면 모든 텍스처가 여기에 표시됩니다.
...
1. 텍스처 아이콘을 선택하면 프로젝트 창에서 텍스처가 선택됩니다. 프로젝트 폴더에 텍스처가 생성되지 않기 때문에 런타임 자료에는 작동하지 않습니다.
1. sRGB 버튼은 [텍스처 가져오기 설정]의 sRGB(색상 텍스처) 옵션과 비슷하게 작동합니다. 감마 공간(sRGB)이나 선형 텍스처를 해석할 것인지 설정할 수 있습니다. Substance 플러그인은 이 해석을 자동으로 처리하지만 필요한 경우 재정의할 수 있습니다.
...
드롭다운 메뉴를 사용하여 다른 텍스처의 알파 텍스처에 채널을 압축할 수 있습니다. 생성된 각 텍스처에는 Substance 재질에 의해 생성된 모든 텍스처 출력의 목록이 포함된 드롭다운 메뉴가 있습니다. 목록에서 맵을 선택하여 텍스처의 알파 채널에 압축하면 됩니다. [소스] 옵션은 텍스처의 알파 채널입니다.

이 이미지에서 저는 높이 맵을 선택했습니다.

![](../../../assets/screen-shot-2022-03-29-at-2-48-33-pm.png)

다음 이미지에서는 Height 출력이 기본 색상 맵의 알파 채널에 압축되고 있음을 확인할 수 있습니다.
