...
| 이름 | 입력 | 설명 |
| --- | --- | --- |
| **GetSubstance** | 재질 | 재질에 사용된 Substance 그래프 인스턴스의 배열을 반환합니다. 서로 다른 두 그래프 인스턴스의 텍스처 출력을 사용하는 재질을 만드는 경우 이 함수는 해당 두 그래프 인스턴스를 반환합니다. |
| **GetSubstanceTexture** | **SubstanceGraphInstance** | Substance 그래프 인스턴스 입력 매개 변수에서 활성화된 모든 텍스처와 현재 계산된 텍스처의 배열을 반환합니다. |
| **GetGraphName** | **SubstanceGraphInstance** | Designer에 설정된 그래프 이름을 반환합니다. |
| **GetFactoryName** | **SubstanceGraphInstance** | 이 노드에 전달된 **SubstanceGraphInstance**&#x200B;을(를) 만드는 데 사용된 **GraphInstanceFactory**&#x200B;의 이름을 반환합니다. |
...
| **인스턴스 출력 사용** | 입력: **SubstanceGraphInstance** - 입력을 활성화하는 출력이 포함된 그래프 인스턴스: **OutputIndexes**(int32 배열) - 활성화할 출력의 인덱스입니다. (변경될 수 있음) | 이전에 사용하지 않도록 설정한 경우 **SubstanceGraphInstance**&#x200B;에 전달된 텍스처 출력을 만듭니다. 이 기능은 **SubstanceGraphInstance** 편집기에서 출력을 활성화하는 것과 동일합니다. *참고: 새로 만든 텍스처로 재질이 업데이트되지 않습니다. 새 출력을 사용하여 런타임에 sampler 매개 변수를 설정하여 처리해야 합니다.* |
| **DisableInstanceOutput** | 입력: **SubstanceGraphInstance** - 값을 적용할 그래프 인스턴스입니다.입력: **SubstanceGraphInstance** - 값을 가져올 그래프 인스턴스입니다. | Substance 그래프 인스턴스 입력 매개 변수의 변경된 모든 입력 값을 복원합니다. |
...
| **비동기 렌더링** | **SubstanceGraphInstance** | Substance 그래프 인스턴스 입력의 출력 텍스처를 다시 계산합니다. (차단 금지) |
| **동기화 렌더링** | **SubstanceGraphInstance** | Substance 그래프 인스턴스 입력의 출력 텍스처를 다시 계산합니다. (차단) |
...
| **SubstanceConnectionStruct** | 입력: **출력 식별자**(FString)*입력에 연결할 텍스처 출력의 식별자입니다.* 입력: **입력 식별자**(FString) | 집계 Substance 팩터리 만들기에서 각 출력 텍스처를 새 입력 텍스처와 연결하는 방법을 지정하는 데 사용됩니다. |
