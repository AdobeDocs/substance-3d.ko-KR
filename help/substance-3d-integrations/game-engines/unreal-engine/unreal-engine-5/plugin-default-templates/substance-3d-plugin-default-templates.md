---
helpx_url: "https://helpx.adobe.com/substance-3d-integrations/game-engines/unreal-engine/unreal-engine-5/substance-3d-plugin-default-templates.html"
breadcrumb-title: ''
description: Unreal Engine 5 워크플로용 Substance 3D 플러그인에서 제공하는 기본 재질 템플릿에 대해 알아봅니다.
helpx_creative_field: ""
helpx_description: Ecosystems and Plugins > Game Engines > Unreal Engine > Unreal Engine 5 > Substance 3D Plugin Default Templates
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Substance 3D 플러그인 기본 템플릿
user-guide-description: ''
user-guide-title: ''
source-git-commit: 0197b6f5f4e3ed1f2bc0e5576bd5818d04485ed5
workflow-type: tm+mt
source-wordcount: '75'
ht-degree: 0%

---


# Substance 3D 플러그인 기본 템플릿

*참고: 이 기능은 현재 개발 중이며 곧 사용할 수 있습니다.*

버전 5.0.0에서는 Substance 엔진에서 지원하는 숫자 출력으로 개선된 재질 템플릿 워크플로 *을(를) 도입했습니다. 재질 템플릿을 사용하면 UE5에서 Substance 재질 셰이더를 구성하는 방법을 정확하게 정의할 수 있습니다.*
