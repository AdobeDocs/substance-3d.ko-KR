...
페인팅하거나 코팅된 금속, 투명 및 투과 재질에 대해 금속성을 낮추는 대신 레이어링(예: 코팅)을 사용합니다.

### 투명 및 투과 재질

투명하고 투과 재질은 빛이 통과할 수 있게 해줍니다. 일반적인 예로는 유리, 많은 액체, 투명 또는 착색된 플라스틱을 들 수 있다. 이러한 문제의 정의 특성은 다음과 같습니다.
...
&#39;투명&#39;과 &#39;투과&#39;의 구분점: &#39;투명&#39;은 실생활, 일상 용어이며, 그것을 통해 볼 수 있다면 무언가가 투명하다. &#39;투과&#39;는 &#39;반투명도&#39;의 동의어입니다. 예를 들어, 결빙 유리는 빛을 통과시키지만(그래서, 그것은 투과), 그것은 투명하지 않습니다 - 우리는 그것을 볼 수 없습니다.
...
* 비등방성: [비등방성]은 원형이 아닌 표면을 따라 정렬하는 대신 밝은 영역 늘리다를 유발하여 코트 레이어의 반사가 표면에 따라 어떻게 달라지는지 설명합니다. 이 효과는 브러싱, 스트리킹 또는 플로우 패턴과 같은 코팅에서의 직접 표면 구조를 나타내는 데 사용됩니다.
...
* 비등방성 탄젠트: 비등방성 값 이상으로 인한 모든 항목의 줄무늬 방향입니다.
...
* 코팅 정상: 코트 레이어를 약간 변형하여 미세한 크기의 형상 모양을 만들 수 있습니다. 예를 들어 재질의 스크래치 또는 빗방울의 모양을 재현하는 데 사용할 수 있습니다.
...
빛의 통과 효과는 종종 &#39;투명도&#39;나 &#39;반투명도&#39; 아래에 느슨하게 그룹화되지만 OpenPBR은 이 두 효과 사이를 명확히 구분합니다. 투과는 빛이 물질을 통과하여 유리, 물 또는 투명한 플라스틱에서 보이는 것과 같은 반대쪽으로 나가는 것을 말합니다. 서브서피스 스캐터링은 재료 안으로 들어오는 빛, 내부에서 산란되는 빛, 그리고 서로 다른 지점에서 빠져나가는 빛을 묘사하며 부드러운 그림자와 내부 색상을 생성합니다.
...
모든 재질이 OpenPBR으로 표현될 수 있는 것은 아니다. 여느 재질 모델과 마찬가지로, OpenPBR은 그저 모델이다. 비록 그것이 이미 상당히 특색이 풍부할지라도, 그것은 존재하거나 상상할 수 있는 무한히 방대하고 무성한 종류의 물질들에 비하면 조잡하게 남아있다. 어떤 모델은 기본적으로 만들 수 있고, 어떤 모델은 늘리다를 더 많이 구축해야 하며, 어떤 모델은 그 모델을 더 제한해야 하고 또 다른 모델들은 여전히 모델이 나타낼 수 없다. 어떤 경우에는 숙련된 예술가는 그럼에도 불구하고 어떤 &#39;속임수&#39;로 괜찮은 결과를 얻을 수 있다; 이것은 일반적으로 비신체적인 선택이 이루어질 때이다. 그러나 모델을 사용하여 수행할 수 있는 작업과 수행할 수 없는 작업을 이해하고 더 간단한 자료나 전용 셰이더와 같은 대체 솔루션이 필요한 시기를 아는 것이 중요합니다.
