...
아티스트에게 실질적인 차이점은 OpenPBR이 *의도*&#x200B;를 유지하는 것을 목표로 한다는 것입니다. 정확한 시각적 유사성이 가능하지 않더라도, 재료의 구조 - 금속인 것, 투과성인 것, 표면이 얼마나 거칠거나 비등방성인 것 - 는 명확하고 전송 가능하다.
...
일상적인 관점에서, OpenPBR을 사용하여 작업하면 자연스럽게 상호 운용성을 지원하는 습관이 장려됩니다.
...
방출은 직접 광을 방출함으로써 표면이 광원으로 작용할 수 있게 한다. 발광은 반사 현상이 아니지만, 발광 재료가 반사 및 투과 특성과 함께 일관되게 정의될 수 있도록 OpenPBR 재질 모델 내에 포함된다.
...
코팅 레이어가 있는 경우 퍼즈를 제외한 다른 모든 레이어 위에 투명한 반사 레이어가 재현됩니다. 이를 통해 코팅제나 특정 유형의 자동차 페인트와 같은 실제 효과를 시뮬레이션할 수 있습니다. 코트 레이어는 0에서 1 사이의 범위로 정의됩니다. 이 값을 0으로 설정하면 코트 레이어가 완전히 비활성화됩니다.
...
그렇게 하는 데는 많은 반복이 필요했고, 3D 지형은 처음에는 매우 단편화되어 다양한 이해 관계자들이 다양한 시각적 표현을 위해 다양한 방법을 탐색하고, 다양한 전면에서 발전을 이루었습니다. 특정 접근 방식이 우수하게 나타나고 다양한 영역의 작업이 융합되기 시작하여 GGX의 출현, 금속-러프 물질 접근, 궁극적으로 OpenPBR이 될 때까지 하나의 솔루션은 다른 솔루션으로 대체될 것입니다.

이와 병행하여 생산 파이프라인은 더욱 상호 연결되게 되었다. 모델링, 텍스처링, 룩 개발, 조명, 렌더링 및 실시간 사용을 위해 애플리케이션 간에 에셋을 이동할 필요가 점점 증가하고 있습니다. 스튜디오는 USD나 MaterialX 등 표준화된 교환 형식에 더 크게 의존하기 시작했고, 재료 설명을 특별히 움직일 수 있는 형식도 유리하다는 것이 분명해졌다.
